...
var CellManager = function() { 
	...
	for (var i = 0; i < board.width/5; i++) {
		this.cells2D[i] = new Array();
		for (var j = 0; j < board.height/5; j++) {
...
CellManager.prototype.nextStep = function(ctx){
	for(var row=0; row<board.width/5; row++){
		for(var col=0; col<board.height/5; col++){
			var count = 0;
			for(var i=row-1; i<=row+1; i++){
				for(var j=col-1; j<=col+1; j++){
					if (i >= 0 && j >= 0 && i < board.width/5 && j < board.height/5 && !(i === row && j === col)){
